...
class DSConv(nn.Module):
    """Dynamic Snake Convolution (DSConv) implementation. Makaledeki yapıya uygun olarak, kernelin yılan gibi
    kıvrılmasına izin verir.
    """
    ...
    def forward(self, x):
        ...
        # Standard Deformable Conv mantığı ile offsetleri uygula
        ...
        # Offset ağırlıklarını normalize etme (opsiyonel ama stabilize için iyi)
        torch.sigmoid(
            offset[:, : self.k * self.k, :, :]
        )  # Mask kısmı (opsiyonel, DSConv genelde sadece offset kullanır)
